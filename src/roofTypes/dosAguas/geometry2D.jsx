/**
 * roofTypes/dosAguas/geometry2D.jsx — Sección transversal 2D (pórtico a 2 aguas)
...
function PressureProfile({ r }) {
  ...
  // Origin at (p=0, z=0): X maps pressure centered, Y grows upward from bottom
  const x0 = m + graphW / 2;  // X position of pressure = 0 (center)
  const y0 = m + graphH;      // Y position of z = 0 (bottom)
  const toX = p => x0 + (p / maxP) * (graphW / 2);
  const toY = z => y0 - (z / maxH2) * graphH;
  ...
  return <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 360 }}>
    {/* Y-axis at pressure=0, X-axis at z=0 — crossing at origin */}
    <line x1={x0} y1={m} x2={x0} y2={y0} stroke={C.dm} strokeWidth={1} />
    <line x1={m} y1={y0} x2={m + graphW} y2={y0} stroke={C.dm} strokeWidth={1} />

    {/* Pressure grid lines and labels */}
    {[-1, -0.5, 0.5, 1].map(f => (
      <g key={f}>
        <line x1={toX(f * maxP)} y1={m} x2={toX(f * maxP)} y2={y0} stroke={C.bd} strokeWidth={0.3} />
        <text x={toX(f * maxP)} y={y0 + 14} fill={C.dm} fontSize={8} textAnchor="middle" fontFamily="monospace">{(f * maxP).toFixed(0)}</text>
      </g>
    ))}
    <text x={x0} y={y0 + 14} fill={C.dm} fontSize={8} textAnchor="middle" fontFamily="monospace">0</text>

    {/* Height grid lines and labels */}
    {[0.25, 0.5, 0.75, 1].map(f => (
      <g key={f}>
        <line x1={m} y1={toY(f * maxH2)} x2={m + graphW} y2={toY(f * maxH2)} stroke={C.bd} strokeWidth={0.3} />
        <text x={x0 - 6} y={toY(f * maxH2) + 3} fill={C.dm} fontSize={8} textAnchor="end" fontFamily="monospace">{(f * maxH2).toFixed(1)}</text>
      </g>
    ))}
    ...
    {/* Axis labels */}
    <text x={m + graphW / 2} y={y0 + 30} fill={C.dm} fontSize={9} textAnchor="middle" fontFamily="monospace">Presion [Pa]</text>
    <text x={x0 - graphW / 2 - 10} y={m + graphH / 2} fill={C.dm} fontSize={9} textAnchor="middle" fontFamily="monospace" transform={`rotate(-90, ${x0 - graphW / 2 - 10}, ${m + graphH / 2})`}>Altura z [m]</text>
    ...
    {/* Reference height marker */}
    <line x1={x0 - 8} y1={toY(r.h)} x2={x0 + 8} y2={toY(r.h)} stroke={C.ac} strokeWidth={2} />
    <text x={x0 + 12} y={toY(r.h) + 3} fill={C.ac} fontSize={8} fontFamily="monospace">h={r.h.toFixed(1)}m</text>
  </svg>;
}

/** Render2D para cubierta a 2 aguas */
    ...
    <div className="relative" onWheel={e => { e.preventDefault(); setZoom(z => Math.max(0.5, Math.min(3, z + (e.deltaY > 0 ? -0.1 : 0.1)))); }}>
      ...
      <div className="overflow-auto" style={{ maxHeight: 400 }}>
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 360 * zoom, transform: `scale(${zoom})`, transformOrigin: 'center center' }}>
          ...
          <line x1={rx} y1={base} x2={rx} y2={base - we} stroke={C.tx} strokeWidth={3} />
          <line x1={lx} y1={base - we} x2={rdg} y2={rdgY} stroke={C.tx} strokeWidth={3} />
          <line x1={rdg} y1={rdgY} x2={rx} y2={base - we} stroke={C.tx} strokeWidth={3} />
          <line x1={rdg} y1={rdgY} x2={rdg} y2={base} stroke={C.dm} strokeWidth={0.6} strokeDasharray="4 3" />
          ...
          {r.theta > 1 && <text x={rdg + 10} y={rdgY + 14} fill={C.w} fontSize={10} fontFamily="monospace" fontWeight="bold">θ={r.theta.toFixed(1)}°</text>}

          {drawDistLoad(lx, base, lx, base - we, ff.w_ww_max, 'l')}
          {drawDistLoad(rx, base, rx, base - we, ff.w_lw_min, 'r')}
          {drawDistLoad(lx, base - we, rdg, rdgY, ff.w_rbv_min, 'l')}
          {drawDistLoad(rdg, rdgY, rx, base - we, ff.w_rsv_min, 'r')}
          ...
          <text x={W / 2} y={14} fill={C.dm} fontSize={10} fontFamily="monospace" textAnchor="middle">
            Portico Tipico — 2 Aguas — {ff.isEmp ? 'Empotrado' : 'Articulado'} — Sep.={ff.sep.toFixed(2)}m
          </text>
        </svg>
      </div>
    </div>

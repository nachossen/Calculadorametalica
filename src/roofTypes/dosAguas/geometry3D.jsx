/**
 * roofTypes/dosAguas/geometry3D.jsx — Vista isométrica 3D (2 aguas)
 * Extraído del componente View3D del monolito — sin cambios de lógica
 */
...
/**
 * @param {{ r: Object, rY?: number, rX?: number }} props
 */
export function Render3D({ r }) {
  ...
  const bh = r.B / 2, lh = r.L / 2, ro2 = r.ro || 0;
  const ridgeX = ro2;
  const v = {
    flb: pr(-bh, 0, -lh), frb: pr(bh, 0, -lh), blb: pr(-bh, 0, lh), brb: pr(bh, 0, lh),
    fle: pr(-bh, r.he, -lh), fre: pr(bh, r.he, -lh), ble: pr(-bh, r.he, lh), bre: pr(bh, r.he, lh),
    fr: pr(ridgeX, r.hc, -lh), br: pr(ridgeX, r.hc, lh),
  };
  ...
  const faces = [
    { pts: [v.flb, v.frb, v.fre, v.fr, v.fle], p: r.pWW.max, lb: 'BV' },
    { pts: [v.blb, v.brb, v.bre, v.br, v.ble], p: r.pLW.max, lb: 'SV' },
    { pts: [v.flb, v.blb, v.ble, v.fle], p: r.pLat.max, lb: 'Lat' },
    { pts: [v.frb, v.brb, v.bre, v.fre], p: r.pLat.min, lb: 'Lat' },
    { pts: [v.fle, v.ble, v.br, v.fr], p: r.pRBVmax.max, lb: 'Cub BV' },
    { pts: [v.fre, v.bre, v.br, v.fr], p: r.pRSV.max, lb: 'Cub SV' },
  ];
  ...
  for (let i = 0; i < nP; i++) {
    ...
    frames.push({
      ...
      el: pr(-bh, r.he, zz), er: pr(bh, r.he, zz),
      ridge: pr(ridgeX, r.hc, zz),
    });
  }
  ...
  return <svg viewBox={`0 0 ${W} ${H}`} className="w-full cursor-grab active:cursor-grabbing" style={{ maxHeight: 320 }}
    ...
    {frames.map((f, i) => <g key={`fr${i}`}>
      ...
      <line x1={f.el.x} y1={f.el.y} x2={f.ridge.x} y2={f.ridge.y} stroke={C.w} strokeWidth={1.5} opacity={.6} />
      <line x1={f.ridge.x} y1={f.ridge.y} x2={f.er.x} y2={f.er.y} stroke={C.w} strokeWidth={1.5} opacity={.6} />
    </g>)}
    ...
    {[
      ...
      { f: faces[4], lb: 'Cub BV', mx: r.pRBVmax.max, mn: r.pRBVmin.min },
      { f: faces[5], lb: 'Cub SV', mx: r.pRSV.max, mn: r.pRSV.min },
    ].map((item, i) => {
    ...
    {(() => {
      ...
      return <g>
        <defs><marker id="w3" viewBox="0 0 10 7" refX="10" refY="3.5" markerWidth="8" markerHeight="6" orient="auto"><polygon points="0 0,10 3.5,0 7" fill={C.ac} /></marker></defs>
        <line x1={s2.x} y1={s2.y} x2={e2.x} y2={e2.y} stroke={C.ac} strokeWidth={2.5} markerEnd="url(#w3)" />
